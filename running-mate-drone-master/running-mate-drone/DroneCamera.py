...
class DroneVisionGUI:
    def __init__(self, drone_object, is_bebop, user_code_to_run, user_args, buffer_size=70, network_caching=200, fps=30, user_draw_window_fn=None):
        """
        ...
        if (self.vision_running):
            ...
            if (img is not None):
                if self.buffer_index == (self.buffer_size-1):
                    # 파일 저장 경로설정
                    filePath = "/Users/gbpark/Desktop/dr-prj/running-mate-drone/running-mate-drone/datasets/"
                    # 파일 이름 설정
                    fileName = "frame%d.png" % self.buffer_index
